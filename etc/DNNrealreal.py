...
class AE_DNN(nn.Module):
    def __init__(self):
        ...
        # Encoder 정의
        self.Encoder = nn.Sequential(
            nn.Linear(18 * 100, 2048), nn.ReLU(), nn.BatchNorm1d(2048),
            nn.Linear(2048, 2048), nn.ReLU(), nn.BatchNorm1d(2048),
...
# 모델 평가: MSE 기반 이상 탐지
def eval_model(test_loader):
    results, residuals, mse_contributions = [], [], []
